...
public class HttpsCerManager {
    ...
    public void setTrustAll() {
        try {
            ...
            sslContext.init(null,new TrustManager[]{SSLUtils.defaultX509TrustManager()},new SecureRandom());
            okHttpBuilder.sslSocketFactory(SSLUtils.fixSSLLowerThanLollipop(sslContext.getSocketFactory()),SSLUtils.defaultX509TrustManager());
            okHttpBuilder.hostnameVerifier(SSLUtils.defaultHostnameVerifier());
        }catch (Exception e){
            e.printStackTrace();
        }
    }
