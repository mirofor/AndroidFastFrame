...
public class DropDownMenuView {
    ...
    public static PopupWindow showCustom(Activity activity, List<DropBeanFlag> dropBeans,
                                         float widthScale, float heightScale, View anchor, int defCheckedPos,
                                         DropAdapter.OnItemClickListener dropAdapterItemClick,
                                         final com.heiko.dropwidget.DropDownMenu.OnDropDismissListener onDropDismissListener) {
        dismiss();
        View root = findRootView(activity);
        final DropAdapter dropAdapter = initRecyclerView(activity, dropBeans, defCheckedPos, root);
        final PopupWindow popupWindow = DropPopupWindowUtil.showAsDropDown(
                activity, root, widthScale, heightScale, anchor);
