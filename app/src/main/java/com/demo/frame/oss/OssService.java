...
public class OssService {
    ...
    public static OssService init (Context context,UIDisplayer displayer) {

        OSSCredentialProvider credentialProvider;
        credentialProvider = new OSSPlainTextAKSKCredentialProvider(OSSConfig.accessKeyId,OSSConfig.accessKeySecret);
        ClientConfiguration conf = new ClientConfiguration();
        conf.setConnectionTimeout(15 * 1000); // 连接超时，默认15秒
        conf.setSocketTimeout(15 * 1000); // socket超时，默认15秒
        conf.setMaxConcurrentRequest(5); // 最大并发请求书，默认5个
        conf.setMaxErrorRetry(2); // 失败后最大重试次数，默认2次
        OSS oss = new OSSClient(context, OSSConfig.endpoint, credentialProvider, conf);
        OSSLog.enableLog();
        return new OssService(oss, OSSConfig.bucket, displayer);
...
//        });
        ...
        OSSAsyncTask task = mOss.asyncPutObject(put, new OSSCompletedCallback<PutObjectRequest, PutObjectResult>() {
            ...
            public void onSuccess(PutObjectRequest request, PutObjectResult result) {
                ...
                Log.d("Result Info", result.toString());
